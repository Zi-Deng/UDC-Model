...
def load_sweep_config(sweep_config_file=None):
    ...
    return {"matrix_cell": {"row": 1, "col": 2}, "experiment": {"description": "Cost matrix sweep"}}
...
def create_class_accuracy_graph(df, output_dir, matrix_row, matrix_col):
    ...
    for i, (class_name, values) in enumerate(class_data.items()):
        ax.plot(
            df["cost_matrix_value"][: len(values)],
            values,
            "-o",
            linewidth=2,
            markersize=6,
            label=f"Class {class_name.split('_')[1]}",
            color=colors[i],
        )
...
def create_sweep_cell_raw_count_graph(df, output_dir, matrix_row, matrix_col):
    ...
    ax.set_title(
        f"Raw Count in Sweep Cell [{matrix_row},{matrix_col}] vs Cost Matrix Value", fontsize=16, fontweight="bold"
    )
    ax.set_xlabel(f"Cost Matrix Value [{matrix_row},{matrix_col}]", fontsize=12)
...
def create_count_percentage_graph(df, output_dir, matrix_row, matrix_col):
    ...
    ax.set_title(
        f"Count Percentage in Sweep Cell [{matrix_row},{matrix_col}] vs Cost Matrix Value",
        fontsize=16,
        fontweight="bold",
    )
    ax.set_xlabel(f"Cost Matrix Value [{matrix_row},{matrix_col}]", fontsize=12)
...
def create_other_cells_raw_count_graph(df, output_dir, matrix_row, matrix_col, num_classes=3):
    ...
    for i, (cell_name, values) in enumerate(other_cells_data.items()):
        ...
        ax.plot(
            ...
            linewidth=2,
            markersize=4,
            label=f"Cell [{row},{col}]",
            color=colors[i],
        )

    ax.set_title(
        f"Raw Counts in Other Confusion Matrix Cells vs Cost Matrix Value [{matrix_row},{matrix_col}]",
        fontsize=16,
        fontweight="bold",
    )
    ax.set_xlabel(f"Cost Matrix Value [{matrix_row},{matrix_col}]", fontsize=12)
...
def create_comprehensive_dashboard(df, output_dir, matrix_row, matrix_col, num_classes=3):
    ...
    fig.suptitle(
        f"Cost Matrix Sweep [{matrix_row},{matrix_col}]: Comprehensive Analysis Dashboard",
        fontsize=24,
        fontweight="bold",
        y=0.98,
    )
    ...
    ax1.plot(
        df["cost_matrix_value"], df["overall_accuracy"], "b-o", linewidth=2, markersize=5, label="Overall Accuracy"
    )
    ...
    if fpr_values and fnr_values:
        ...
        normalized_costs = (
            (cost_values - cost_min) / (cost_max - cost_min) if cost_max > cost_min else np.zeros_like(cost_values)
        )

        # Create scatter plot with blue color gradient
        scatter = ax7.scatter(
            fnr_values, fpr_values, c=normalized_costs, cmap="Blues", s=80, alpha=0.7, edgecolors="black", linewidth=0.5
        )
...
def create_summary_statistics(df, output_dir, target_class, matrix_row, matrix_col):
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(
            f"Cost Matrix Cell [{matrix_row},{matrix_col}] varied from {df['cost_matrix_value'].min()} to {df['cost_matrix_value'].max()}\n"
        )
        ...
        f.write("-" * 30 + "\n")
        f.write(
            f"Best Overall Accuracy: {best_overall_accuracy['overall_accuracy']:.4f} at cost value {best_overall_accuracy['cost_matrix_value']}\n"
        )
        f.write(
            f"Best Overall F1 Score: {best_overall_f1['overall_f1']:.4f} at cost value {best_overall_f1['cost_matrix_value']}\n\n"
        )
...
def create_fpr_fnr_scatter_plot(df, output_dir, matrix_row, matrix_col, num_classes=3):
    ...
    # Create scatter plot with blue color gradient
    scatter = ax.scatter(
        fnr_values, fpr_values, c=normalized_costs, cmap="Blues", s=100, alpha=0.7, edgecolors="black", linewidth=0.5
    )
    ...
    ax.set_title(
        f"ROC-like Analysis: Class {matrix_row} vs Cost Matrix Value [{matrix_row},{matrix_col}]",
        fontsize=16,
        fontweight="bold",
    )
