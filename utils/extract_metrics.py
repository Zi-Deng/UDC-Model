...
def load_sweep_config(sweep_config_file=None):
    ...
    return {"matrix_cell": {"row": 1, "col": 2}}
    ...
    try:
        ...
        if cm.size > 0:
            ...
            if len(cm) > target_class:
                class_tp = (
                    cm[target_class, target_class] if cm.shape[0] > target_class and cm.shape[1] > target_class else 0
                )
                ...
                confusion_matrix_raw = (
                    cm[matrix_row, matrix_col] if cm.shape[0] > matrix_row and cm.shape[1] > matrix_col else 0
                )
                ...
                confusion_matrix_precision = (
                    cm_precision[matrix_row, matrix_col]
                    if cm_precision.shape[0] > matrix_row and cm_precision.shape[1] > matrix_col
                    else 0.0
                )
                confusion_matrix_recall = (
                    cm_recall[matrix_row, matrix_col]
                    if cm_recall.shape[0] > matrix_row and cm_recall.shape[1] > matrix_col
                    else 0.0
                )
            else:
