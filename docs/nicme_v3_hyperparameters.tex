...
\hypersetup{
  ...
  urlcolor=nicmeTeal,
  citecolor=nicmeBlue,
  pdftitle={NICME V3 Hyperparameters},
...
\begin{center}
  {\huge\bfseries\color{nicmeBlue} NICME V3 Hyperparameters}\\[0.25em]
...
\code{nicme\_logit\_cost\_scale} is the V3 pairwise cost-margin strength: it controls how much extra raw-logit margin the model must learn against costly wrong classes.
...
\begin{itemize}
  ...
  \item \pathref{utils/loss\_functions.py:434--458} implements the V3 pairwise adjustment and V3 hybrid loss.
  \item \pathref{utils/loss\_functions.py:610--649} dispatches \code{nicme\_v3\_logit\_adjustment} and \code{nicme\_v3\_hybrid}.
...
\section{\code{nicme\_logit\_cost\_scale}: V3 Pairwise Cost-Margin Strength}

NICME V3 first constructs a pairwise cost-conditioned offset:
...
The V3 pairwise CE loss is ordinary cross entropy on these adjusted logits:

\begin{equation}
\mathcal{L}_{\mathrm{V3\text{-}CE}}
=
-\log
\frac{\exp(z'_y)}
{\sum_j \exp(z'_j)}.
...
\begin{tabular}{ccc}
\toprule
Cost & Offset before scaling & V3 interpretation \\
\midrule
$M=1$ & $\log(1)=0$ & Ordinary noncritical pairs receive no V3 margin. \\
...
\begin{equation}
\frac{\partial \mathcal{L}_{\mathrm{V3\text{-}CE}}}{\partial z_k}
=
p'_k - \mathbf{1}[k=y].
\end{equation}
...
The hybrid V3 loss adds a second term that uses the original, unadjusted logits:
...
\begin{equation}
\boxed{
\mathcal{L}_{\mathrm{NICME\text{-}V3\text{-}Hybrid}}
=
\mathcal{L}_{\mathrm{V3\text{-}CE}}
+
\lambda_{\mathrm{eff}}(t)\,
...
The V3 CE term shapes the training challenge by inflating costly wrong-class logits inside the loss.
...
\midrule
$\alpha=0$ and $\lambda=0$ & V3 hybrid exactly reduces to ordinary CE. \\
$M[y,k]=1$ for all non-target $k$ & V3 pairwise adjustment matches CE because $\log(1)=0$. \\
Correctly classified sample with high-cost wrong class & V3 still applies, so the model is pushed to improve the margin. \\
...
NICME V3 generalizes the shape of that idea:
...
NICME V3 & Pairwise cost $M[y,k]$ for true class $y$ and candidate wrong class $k$. \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Why V3 Is TPT-Aware, Not TPT-Proof}
...
NICME V3 addresses that critique by applying cost-conditioned margins unconditionally, including on samples that are already correctly classified.

However, it would be too strong to claim that V3 is theoretically TPT-proof.
...
\keybox{
NICME V3 is a TPT-aware unconditional pairwise cost-margin loss with an expected-cost regularizer.
...
The selected final V3 run currently records:
...
\noindent Source:
\begin{flushleft}
\pathref{results/pmi10\_v3\_balanced\_convnext\_base\_20260502/final/}\\
...
\noindent Source:
\begin{flushleft}
\pathref{results/pmi10\_v3\_balanced\_convnext\_base\_20260502/hpo-v3/configs/}\\
