...
\hypersetup{
  ...
  urlcolor=nicmeTeal,
  citecolor=nicmeBlue,
  pdftitle={Why NICME V3 Can Outperform CSADA on PMI-10},
...
\begin{center}
  {\huge\bfseries\color{nicmeBlue} Why NICME V3 Can Outperform CSADA}\\[0.25em]
...
\keybox{
\textbf{Thesis.}
For the completed PMI-10 balanced no-calibration experiment, NICME V3 can plausibly outperform CSADA on the clean-test, recall-first, cost-sensitive tradeoff because its loss directly modifies the clean logits in two aligned ways: it imposes pairwise raw-logit margins proportional to the expert cost matrix, and it directly penalizes clean softmax probability mass on costly wrong classes.
...
This is not a universal theorem that NICME V3 dominates CSADA.
It is a theoretical and empirical explanation for the observed repository result:
NICME V3 run 95 ties the previous CSADA result on target-min and target-macro recall, but has much lower normalized ATC and much higher balanced accuracy.
...
\begin{equation}
  ...
  \label{eq:v3-adjusted-logits}
\end{equation}
where $\alpha=\code{nicme\_logit\_cost\_scale}$.
The V3 pairwise CE component is
\begin{align}
  L_{\mathrm{V3CE}}(z,y)
  &=
  -\log
...
For any tolerance $\rho>0$, the odds contribution of class $k$ relative to the target in the NICME V3 softmax satisfies
\begin{equation}
...
Therefore NICME V3 can keep increasing safety margins even after ordinary CE would already view the example as easy.
...
NICME V3 has two clean-data mechanisms.
...
The point is narrower: CSADA's cost signal is mediated by local perturbation geometry, whereas NICME V3's cost signal is explicit in clean logit space.
\end{remark}
...
\section{Why NICME V3 Can Win On PMI-10}
...
NICME V3 is exactly a cost-conditioned logit-geometry method.
...
\begin{enumerate}
  ...
  NICME V3 changes the source of the margin from class priors to the pairwise cost matrix.
  ...
  NICME V3 instead makes the clean loss demand a larger margin against costly wrong classes by construction.
  ...
  In this run, the empirical fingerprint is consistent with that risk: CSADA attains excellent cared-class recall but much lower balanced accuracy than NICME V3.
\end{enumerate}
...
The best observed NICME V3 HPO test row is run 95 with $\alpha=0.5$ and $\lambda=0.07$.

\begin{center}
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{0.32\linewidth}rrr}
\toprule
Metric & CSADA baseline & NICME V3 run 95 & Direction \\
\midrule
...
\begin{itemize}
  \item \pathref{results/pmi10\_nicme\_v3\_pretty\_alpha\_lambda\_lr5e5\_20260503/post\_hpo\_sota\_comparison.md}
  \item \pathref{results/pmi10\_nicme\_v3\_pretty\_alpha\_lambda\_lr5e5\_20260503/grid\_run\_ledger.csv}
...
NICME V3 run 95 is the best observed recall-first cost-sensitive tradeoff among the completed repository baselines and completed HPO rows:
...
\begin{enumerate}
  ...
  \item NICME V3's advantage disappears when run 95 is not allowed to be selected after inspecting test metrics.
  ...
  \item Measuring margins shows no larger $z_y-z_k$ gaps on high-cost PMI pairs for NICME V3 than for CSADA.
\end{enumerate}

\section{Conclusion}

NICME V3 and CSADA solve related but not identical optimization problems.
...
NICME V3 instead changes the clean training objective so that high-cost wrong classes require larger raw margins and receive direct probability-mass penalties.
...
NICME V3 can preserve CSADA's cared-class recall while reducing expected cost and avoiding the clean balanced-accuracy degradation seen in the CSADA row.
