...
#
# Iterates through different cost matrix values for a specified cell,
# runs training for each value, extracts metrics, and generates analysis
# graphs.  Results are saved to the directory specified in the sweep config.
#
# Usage:
#   micromamba activate ml
#   bash run_cost_matrix_sweep.sh                               # uses default config
#   bash run_cost_matrix_sweep.sh config/sweep_2class_bw_cost.json

set -euo pipefail

# Default sweep configuration file
SWEEP_CONFIG_FILE="config/sweep_2class_bw_cost.json"
...
# Backup original config and ensure it is restored on exit/interrupt
...
python utils/extract_metrics.py --print-header --sweep-config "$SWEEP_CONFIG_FILE" dummy_dir 0 > "$METRICS_CSV"
...
extract_metrics() {
    ...
    if python utils/extract_metrics.py "$results_dir" "$cost_value" --output "$METRICS_CSV" --sweep-config "$SWEEP_CONFIG_FILE"; then
        return 0
    else
...
# Main iteration loop
cost_values=($(python -c "import numpy as np; print(' '.join([str(x) for x in np.arange($COST_MIN, $COST_MAX + $COST_STEP/2, $COST_STEP)]))"))
...
for cost_value in "${cost_values[@]}"; do
    ...
    if python scripts/train.py --config "$CONFIG_FILE" > "$log_file" 2>&1; then
        ...
        if [[ -n "$results_dir" && -d "$results_dir" ]]; then
            ...
            if [[ "$UPDATE_GRAPHS" == "true" ]]; then
                ...
                if python utils/analyze_cost_matrix_results.py "$RESULTS_DIR" --sweep-config "$SWEEP_CONFIG_FILE"; then
...
if [[ "$GENERATE_FINAL" == "true" ]]; then
    ...
    python utils/analyze_cost_matrix_results.py "$RESULTS_DIR" --final --sweep-config "$SWEEP_CONFIG_FILE"
else
