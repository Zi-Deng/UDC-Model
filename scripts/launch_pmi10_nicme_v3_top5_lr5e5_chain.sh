...
OUT="${PMI10_NICME_V3_TOP5_LR5E5_OUT:-${ROOT}/results/pmi10_nicme_v3_top5_alpha_lambda_lr5e5_multiseed_20260504}"
SEEDS="${PMI10_NICME_V3_TOP5_SEEDS:-42,43,44}"
SPLIT_DIR="${ROOT}/data/prepared/pmi_pills_10_no_cal/splits/balanced"
PYTHONPATH="${ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
export PYTHONPATH

CHAIN_DIR="${OUT}/chain"
LOG="${CHAIN_DIR}/nicme_v3_top5_lr5e5_chain.log"
...
runner_cmd() {
  local phase="$1"
  shift
  micromamba run -n ml python scripts/run_pmi10_nicme_v3_top5_lr5e5.py \
    --phase "${phase}" \
    --split-dir "${SPLIT_DIR}" \
    --output-root "${OUT}" \
    --seeds "${SEEDS}" \
...
run_phase "unit-tests" \
  micromamba run -n ml pytest -q \
    ...
    tests/test_pmi10_nicme_v3_top5_runner.py
...
run_phase "long-nicme-v3-top5-chain" \
