...
OUT="${PMI20_NICME_ALPHA_LAMBDA6_LR5E5_OUT:-${ROOT}/results/pmi20_nicme_alpha_lambda6_lr5e5_multiseed_20260504}"
SEEDS="${PMI20_NICME_ALPHA_LAMBDA6_SEEDS:-42,43,44}"
SPLIT_DIR="${ROOT}/data/prepared/pmi_pills/splits/balanced"
PYTHONPATH="${ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
export PYTHONPATH

CHAIN_DIR="${OUT}/chain"
LOG="${CHAIN_DIR}/pmi20_nicme_alpha_lambda6_lr5e5_chain.log"
...
runner_cmd() {
  local phase="$1"
  shift
  micromamba run -n ml python scripts/run_pmi20_nicme_alpha_lambda6_lr5e5.py \
    --phase "${phase}" \
    --split-dir "${SPLIT_DIR}" \
    --output-root "${OUT}" \
    --seeds "${SEEDS}" \
...
run_phase "unit-tests" \
  micromamba run -n ml pytest -q \
    ...
    tests/test_pmi20_nicme_alpha_lambda6_runner.py
...
run_phase "long-pmi20-nicme-alpha-lambda6-chain" \
