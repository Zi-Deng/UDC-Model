"""PMI-10 NICME v3 top-5 alpha/lambda multi-seed runner.

This runner repeats the five best observed LR 5e-5 NICME v3 HPO settings
...
DEFAULT_OUTPUT_ROOT = Path("results/pmi10_nicme_v3_top5_alpha_lambda_lr5e5_multiseed_20260504")
...
def method_slug(point: dict[str, Any]) -> str:
    return (
        f"nicme_v3_run{point['hpo_run']}_"
...
def build_training_config(
    ...
    cfg["sota_role"] = "nicme_v3_top5_alpha_lambda_multiseed"
    cfg["sota_comparison_profile"] = "nicme_v3_top5_alpha_lambda_lr5e5_multiseed"
...
def write_winner_report(output_root: Path, winners: list[dict[str, Any]]) -> None:
    lines = [
        "# NICME v3 Top-5 Cost-Sensitive Winners",
        "",
...
def write_claim_audit(output_root: Path, winners: list[dict[str, Any]], aggregate_rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# NICME v3 Top-5 Claim Audit",
        "",
        "This analysis compares only five NICME v3 alpha/lambda candidates selected from pilot HPO.",
...
def write_method_hyperparameters(output_root: Path, aggregate_rows: list[dict[str, Any]]) -> None:
    lines = [
        "# NICME v3 Top-5 Hyperparameters",
        "",
...
def write_readme(output_root: Path, args: argparse.Namespace, aggregate_rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# PMI-10 NICME v3 Top-5 Alpha/Lambda Multi-Seed Results",
        "",
        f"Generated: {dt.datetime.now().isoformat(timespec='seconds')}",
        "",
        "This folder contains a fixed-protocol robustness rerun for the top five observed NICME v3 alpha/lambda settings from the LR 5e-5 pilot HPO.",
        "",
        "## Protocol",
        "",
...
def assert_top5_config(cfg: dict[str, Any]) -> None:
    if cfg["loss_function"] != "nicme_v3_hybrid":
        raise ValueError("Top-5 runner must use nicme_v3_hybrid")
...
def run_preflight(args: argparse.Namespace) -> None:
    ...
    print("NICME v3 top-5 PMI-10 preflight passed")
