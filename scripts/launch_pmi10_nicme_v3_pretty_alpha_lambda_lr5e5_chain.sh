...
OUT="${PMI10_NICME_V3_PRETTY_GRID_OUT:-${ROOT}/results/pmi10_nicme_v3_pretty_alpha_lambda_lr5e5_20260503}"
SPLIT_DIR="${ROOT}/data/prepared/pmi_pills_10_no_cal/splits/balanced"
PYTHONPATH="${ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
export PYTHONPATH

CHAIN_DIR="${OUT}/chain"
LOG="${CHAIN_DIR}/nicme_v3_pretty_alpha_lambda_grid.log"
...
run_phase "grid-dry-run" \
  runner_cmd grid-dry-run

run_phase "grid-smoke" \
  runner_cmd grid-smoke --per-run-timeout-minutes 240

run_phase "grid-run" \
  runner_cmd grid-run --per-run-timeout-minutes 960

run_phase "grid-analyze" \
  runner_cmd grid-analyze
