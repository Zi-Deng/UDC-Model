...
OUT="${PMI10_CAMERA_READY_LR5E5_OUT:-${ROOT}/results/pmi10_camera_ready_lr5e5_multiseed_20260504}"
SEEDS="${PMI10_CAMERA_READY_SEEDS:-42,43,44}"
SPLIT_DIR="${ROOT}/data/prepared/pmi_pills_10_no_cal/splits/balanced"
PYTHONPATH="${ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
export PYTHONPATH

CHAIN_DIR="${OUT}/chain"
LOG="${CHAIN_DIR}/camera_ready_lr5e5_chain.log"
...
runner_cmd() {
  local phase="$1"
  shift
  micromamba run -n ml python scripts/run_pmi10_camera_ready_lr5e5.py \
    --phase "${phase}" \
    --split-dir "${SPLIT_DIR}" \
    --output-root "${OUT}" \
    --seeds "${SEEDS}" \
...
run_phase "long-camera-ready-chain" \
